"""
Ontology manager for Keboola metadata.
...
class OntologyManager:
    """
    Manager for the Keboola metadata ontology.
    ...
    def __init__(self, state_dir: Optional[str] = None):
        ...
        storage_path = os.path.join(self.state_dir, "ontology.ttl")
        self.triple_store = TripleStore(storage_path=storage_path)
    ...
    def get_relationships_for_entity(self, entity_id: str, direction: str = "both") -> List[Relationship]:
        """
    ...
    def save_state(self) -> None:
        ...
        try:
            # Save entities
            entities_path = os.path.join(self.state_dir, "entities.json")
            ...
            # Save relationships
            relationships_path = os.path.join(self.state_dir, "relationships.json")
    ...
    def visualize(self, output_path: Optional[str] = None, format: str = "png"):
        """
        Visualize the ontology as a graph.
        ...
        try:
            ...
            G = self.to_networkx()
            if not G:
                logger.error("Failed to create NetworkX graph")
                return None
            
            if not output_path:
                output_path = os.path.join(self.state_dir, f"ontology.{format}")
            ...
            logger.info(f"Saved ontology visualization to {output_path}")
            return output_path
        except ImportError:
            logger.error("NetworkX or pygraphviz not available, cannot visualize")
            return None
        except Exception as e:
