...
class QdrantIndexer:
    ...
    def index_metadata(
        ...
        logging.info(f"Starting indexing of {total_items} total items with batch size {batch_size}")
            ...
            if isinstance(items, list):
                ...
                if metadata_type == 'tables':
                    ...
                    for bucket_id, tables in items.items():
                        logging.info(f"Processing {len(tables)} tables for bucket {bucket_id}")
                        self._index_items(
                            tables, 
                            metadata_type, 
                            embedding_provider, 
                    ...
                    logging.info(f"Found {len(item_list)} items of type {metadata_type}")
                    self._index_items(
                        item_list,
                        metadata_type,
                        embedding_provider,
    ...
    def _index_items(
        ...
        for i in range(0, len(items), batch_size):
            ...
            try:
                ...
                texts = [
                    self._prepare_text_for_embedding(item)
                    for item in batch
                ]
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if "No space left on device" in str(e) and attempt < max_retries - 1:
                            logging.warning(f"Storage issue, retrying with smaller batch... (attempt {attempt + 1})")
